...
export function Sidebar() {
  ...
  return (
    <aside className="flex flex-col h-full bg-[var(--color-bg-secondary)] border-r border-[var(--color-border)] select-none">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1">
        ...
        {APPS.map((app) => {
          const isActive = location.pathname.startsWith(app.route)
          return (
            <button
              key={app.id}
      ...
      <div className="px-2 py-2 border-t border-[var(--color-border)]">
        <button
          ...
          className="flex items-center gap-2.5 w-full px-2.5 py-1.5 rounded-lg text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)] transition-colors cursor-pointer"
        >
          <Settings size={16} />
          <span>Settings</span>
        </button>
